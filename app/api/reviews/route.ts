...
export async function DELETE(request: Request) {
  ...
  if (!placeId) return jsonError("店舗が指定されていません");
  const DB = await ensureDatabase();
  const review = await DB.prepare(`SELECT id, is_seed as isSeed, image_key as imageKey FROM reviews WHERE user_id=? AND place_id=?`).bind(user.id, placeId).first<{ id: string; isSeed: number; imageKey: string | null }>();
  ...
  await DB.batch([
    ...
    DB.prepare(`DELETE FROM tier_entries WHERE user_id=? AND place_id=? AND is_seed=0`).bind(user.id, placeId),
  ]);
  if (review.imageKey) await bindings().MEDIA.delete(review.imageKey).catch(() => undefined);
  return Response.json({ ok: true });
}
