...
type Lookups = { usersById: Map<string, User>; placesById: Map<string, Place>; productsById: Map<string, Product>; reviewsByPlace: Map<string, Review[]>; productReviewsByProduct: Map<string, ProductReview[]>; reviewsByUser: Map<string, Review[]>; tiersByUser: Map<string, TierEntry[]> };
...
export function KokoumaApp() {
  ...
  const lookups = useMemo<Lookups>(() => {
    ...
    return { usersById, placesById, productsById, reviewsByPlace, productReviewsByProduct, reviewsByUser, tiersByUser };
  }, [data]);
  ...
  return <div className="app-shell">
    ...
    {view === "discover" && <main className="discover-layout">
      <section className={`feed-panel ${mobileMap ? "mobile-hidden" : ""}`}>
        ...
        <div className="filters reveal-2"><button className={followingOnly ? "active friend-filter" : "friend-filter"} onClick={() => current ? setFollowingOnly(!followingOnly) : setModal("auth")}><Heart size={14} fill={followingOnly ? "currentColor" : "none"} />フォロー中</button></div>
...
function ProfileView({user,data,lookups,onQr,onLogout,onSelectPlace}:{user:User;data:Bootstrap;lookups:Lookups;onQr:()=>void;onLogout:()=>void;onSelectPlace:(id:string)=>void}){void lookups;return <main className="own-profile"><div className="own-head"><ProfileCard user={user} data={data} followed={false} self onFollow={()=>{}}/><div className="own-tools"><button onClick={onQr}><QrCode/>友達追加QRを表示</button><button onClick={onLogout}>ログアウト</button></div></div><div className="my-picks"><span className="eyebrow">MY FOOD MEMORY</span><h2>わたしの記録</h2>{data.reviews.filter((r)=>r.userId===user.id).length?<div className="my-pick-grid">{data.reviews.filter((r)=>r.userId===user.id).map((review)=>{const place=data.places.find((p)=>p.id===review.placeId)!;return <button key={review.id} onClick={()=>onSelectPlace(place.id)}><span style={{backgroundImage:`url(${place.imageUrl})`}}/><b>{place.name}</b><small>★ {review.rating}</small></button>})}</div>:<div className="empty-state"><Star/><h3>最初のレビューを残しましょう。</h3><p>見つける画面からお店を選んでください。</p></div>}</div></main>}
