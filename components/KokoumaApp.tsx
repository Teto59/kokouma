...
export function KokoumaApp() {
  ...
  const areas = useMemo(() => ["すべて", ...new Set((data?.places ?? []).map((place) => place.area).filter(Boolean))], [data]);
  ...
  return <div className="app-shell">
    ...
    {view === "people" && <PeopleView data={data!} current={current} followed={followed} selected={selectedProfile} focused={profileFocus&&Boolean(selectedProfile)} onSelect={openProfile} onExitFocus={() => { setProfileFocus(false); setProfileId(undefined); const url=new URL(location.href); url.searchParams.delete("profile"); history.replaceState(null,"",url); window.scrollTo({top:0}); }} onFollow={async (id) => { if (!current) return setModal("auth"); try { await api("/api/follows", { method:"POST", headers:{"content-type":"application/json"}, body:JSON.stringify({userId:id}) }); await refresh(); } catch(e) { showToast((e as Error).message); } }} onQr={() => requireAuth("qr")} onScan={() => setModal("scan")} />}
    ...
    {selectedPlace && <PlaceDrawer place={selectedPlace} data={data!} current={current} onClose={() => setSelectedPlaceId(undefined)} onReview={() => requireAuth("review")} onDelete={() => setModal("delete-place")} onProfile={(id) => { openProfile(id); setSelectedPlaceId(undefined); }} />}
    ...
    {modal === "add" && current && <AddPlaceModal existingAreas={areas.filter((value) => value !== "すべて")} onClose={() => setModal(null)} onDone={async (id) => { setModal(null); await refresh(); setSelectedPlaceId(id); showToast("地図に新しい一軒を追加しました"); }} />}
    {modal === "review" && selectedPlace && current && <ReviewModal place={selectedPlace} existing={data!.reviews.find((review) => review.placeId === selectedPlace.id && review.userId === current.id)} existingTier={data!.tiers.find((entry) => entry.placeId === selectedPlace.id && entry.userId === current.id)?.tier} onClose={() => setModal(null)} onDone={async () => { setModal(null); await refresh(); showToast("レビューを記録しました"); }} />}
    {modal === "delete-place" && selectedPlace && <DeletePlaceModal place={selectedPlace} onClose={() => setModal(null)} onDone={async () => { setModal(null); setSelectedPlaceId(undefined); await refresh(); showToast("登録したお店を削除しました"); }} />}
...
function AddPlaceModal({ existingAreas, onClose, onDone }: { existingAreas: string[]; onClose: () => void; onDone: (id: string) => void }) {
  const [step,setStep]=useState(1); const [busy,setBusy]=useState(false); const [error,setError]=useState(""); const [link,setLink]=useState(""); const [photo,setPhoto]=useState<File>(); const [draft,setDraft]=useState({ name:"",address:"",area:"",category:"カフェ",latitude:35.7122,longitude:139.7947,googleMapsUrl:"",imageKey:"" });
  ...
  const [reviewBody,setReviewBody]=useState(""); const [reviewRating,setReviewRating]=useState(5); const [reviewTier,setReviewTier]=useState<"S"|"A"|"B"|"C">("S"); const [reviewVisibility,setReviewVisibility]=useState<ReviewVisibility>("public");
  ...
  const resolve = async () => { setBusy(true); setError(""); try { const result=await api("/api/places/resolve",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({url:link})}); const latitude=result.latitude??35.7122; const longitude=result.longitude??139.7947; setDraft((d)=>({...d,name:result.name||d.name,latitude,longitude,googleMapsUrl:link})); setStep(2); if(result.latitude!=null&&result.longitude!=null) await inferArea(latitude,longitude); } catch(e){setError((e as Error).message);} finally{setBusy(false);} };
  ...
  return <Modal title="新しい一軒を地図へ。" onClose={onClose} wide><div className="steps"><span className={step>=1?"active":""}>1 <b>LINK</b></span><i/><span className={step>=2?"active":""}>2 <b>CHECK</b></span><i/><span className={step>=3?"active":""}>3 <b>PHOTO + NOTE</b></span></div>{step===1&&<div className="link-step"><div className="maps-link-visual"><MapPin/><span>Google Maps</span><i>SHARE LINK</i></div><p>Google Mapsでお店を開き、「共有」からコピーしたリンクを貼ってください。</p><label className="big-input"><input value={link} onChange={(e)=>setLink(e.target.value)} placeholder="https://maps.app.goo.gl/…"/><button onClick={resolve} disabled={busy||!link}>{busy?<LoaderCircle className="spin"/>:<ChevronRight/>}</button></label>{error&&<p className="form-error">{error}</p>}<small>リンクから位置を取得できない場合も、次の画面でピンを補正できます。</small></div>}{step===2&&<div className="check-grid"><div className="mini-map"><MapPanel places={[{...draft,id:"draft",createdBy:"",averageRating:0,reviewCount:0,isSeed:false}]} selected="draft" onSelect={()=>{}} onPick={pick} center={[draft.longitude,draft.latitude]}/><p><MapPin/>ピンを動かしたら右側で確定</p></div><div className="form-stack"><label>店名<input value={draft.name} onChange={(e)=>setDraft({...draft,name:e.target.value})} placeholder="お店の名前"/></label><label>住所 <small className="optional-mark">任意</small><input value={draft.address} onChange={(e)=>setDraft({...draft,address:e.target.value})} placeholder="地図から自動入力・空欄でもOK"/></label><div className="area-field"><label><span className="area-label-row">エリア {areaStatus==="auto"&&<small>地図から自動</small>}</span><div className="area-input-row"><input value={draft.area} list="kokouma-area-suggestions" onChange={(e)=>{setDraft({...draft,area:e.target.value});setAreaStatus("manual")}} maxLength={40} placeholder="例：浅草"/><button type="button" onClick={()=>inferArea(draft.latitude,draft.longitude)} disabled={areaStatus==="loading"}>{areaStatus==="loading"?<LoaderCircle className="spin"/>:<Crosshair/>}{areaStatus==="stale"?"このピンで決定":"ピンから取得"}</button></div><datalist id="kokouma-area-suggestions">{allAreaSuggestions.map((value)=><option key={value} value={value}/>)}</datalist><span className={`area-help status-${areaStatus}`}>{areaStatus==="auto"?"地図のピンから推定しました。違う場合は修正できます。":areaStatus==="stale"?"ピンの位置が変わりました。エリアを取得し直してください。":areaStatus==="error"?"自動取得できませんでした。エリア名を直接入力してください。":"入力すると、登録済みのエリアも候補に出ます。"}</span></label></div><label>ジャンル<input value={draft.category} onChange={(e)=>setDraft({...draft,category:e.target.value})}/></label><button className="primary-submit" onClick={()=>setStep(3)} disabled={!draft.name||!draft.area.trim()}>写真へ進む<ChevronRight/></button></div></div>}{step===3&&<div className="photo-step"><div className="photo-review-grid"><label className="photo-drop"><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(e)=>setPhoto(e.target.files?.[0])}/>{photo?<><img src={URL.createObjectURL(photo)} alt="プレビュー"/><span>写真を変更</span></>:<><Camera/><b>お店の空気が伝わる一枚</b><small>JPEG / PNG / WebP · 最大5MB · 任意</small></>}</label><section className="quick-review"><header><span>ONE-TAP MEMORY</span><h3>ついでにレビュー <small>任意</small></h3><p>ここで書けば、登録後にもう一度開く必要はありません。</p></header><div className="quick-review-score"><div><p>おすすめ度</p><Stars value={reviewRating} interactive onChange={setReviewRating}/></div><div><p>あなた的Tier</p><div className="compact-tier">{(["S","A","B","C"] as const).map((value)=><button type="button" key={value} className={reviewTier===value?"active":""} style={{"--tier":tierColors[value]} as React.CSSProperties} onClick={()=>setReviewTier(value)}>{value}</button>)}</div></div></div><label>ひとこと<textarea value={reviewBody} onChange={(event)=>setReviewBody(event.target.value)} maxLength={600} placeholder="また行きたい理由をひとこと。空欄なら店舗だけ登録します。"/><small>{reviewBody.length}/600</small></label><fieldset className="quick-audience"><legend>公開範囲</legend>{(Object.keys(visibilityCopy) as ReviewVisibility[]).map((value)=><button type="button" key={value} className={reviewVisibility===value?"active":""} onClick={()=>setReviewVisibility(value)} aria-pressed={reviewVisibility===value}>{visibilityCopy[value].label}</button>)}</fieldset></section></div>{error&&<p className="form-error">{error}</p>}<div className="modal-footer"><button className="ghost-button" onClick={()=>setStep(2)}><ChevronLeft/>戻る</button><button className="primary-submit" onClick={save} disabled={busy||reviewBody.trim().length===1}>{busy?<LoaderCircle className="spin"/>:reviewBody.trim()?"お店とレビューを登録":"このお店だけ登録"}</button></div></div>}</Modal>;
}

function ReviewModal({ place,existing,existingTier,onClose,onDone }: { place:Place;existing?:Review;existingTier?:TierEntry["tier"];onClose:()=>void;onDone:()=>void }) {
  const [rating,setRating]=useState(existing?.rating??5); const [body,setBody]=useState(existing?.body??""); const [tier,setTier]=useState<TierEntry["tier"]>(existingTier??"S"); const [visibility,setVisibility]=useState<ReviewVisibility>(existing?.visibility??"public"); const [busy,setBusy]=useState(false); const [error,setError]=useState(""); const [photo,setPhoto]=useState<File>(); const [removePhoto,setRemovePhoto]=useState(false); const photoInput=useRef<HTMLInputElement>(null);
  ...
  return <Modal title={existing?`${place.name}の記録を編集`:`${place.name}の記録`} onClose={onClose}><div className="review-form"><p>あなたの星</p><Stars value={rating} interactive onChange={setRating}/><div className="review-photo-editor"><label><input ref={photoInput} type="file" accept="image/jpeg,image/png,image/webp" onChange={(event)=>{setPhoto(event.target.files?.[0]);setRemovePhoto(false)}}/>{photoUrl?<img src={photoUrl} alt="レビュー写真のプレビュー"/>:<><ImagePlus/><span><b>食べたもの・お店の写真</b><small>タップして追加 · 任意</small></span></>}</label>{photoUrl&&<div><button type="button" onClick={()=>photoInput.current?.click()}><Camera/>写真を変更</button><button type="button" className="remove-review-photo" onClick={()=>{setPhoto(undefined);setRemovePhoto(true)}}><Trash2/>写真を削除</button></div>}</div><label>ひとこと<textarea value={body} onChange={(e)=>setBody(e.target.value)} maxLength={600} placeholder="誰と、どんな日に行きたい？ 味だけでなく、あなたの記憶を。"/><small>{body.length}/600</small></label><div><p>あなた的Tier</p><div className="tier-picker">{(["S","A","B","C"] as const).map((t)=><button key={t} className={tier===t?"active":""} style={{"--tier":tierColors[t]} as React.CSSProperties} onClick={()=>setTier(t)}>{t}<small>TIER</small></button>)}</div></div><fieldset className="audience-picker"><legend>この記録を見せる人</legend>{(Object.keys(visibilityCopy) as ReviewVisibility[]).map((value)=><button type="button" key={value} className={visibility===value?"active":""} onClick={()=>setVisibility(value)} aria-pressed={visibility===value}>{value === "public" ? <Share2 /> : value === "following" ? <Heart /> : <Users />}<span><b>{visibilityCopy[value].label}</b><small>{visibilityCopy[value].detail}</small></span></button>)}<p><Users />対象外の人には、レビュー本文・星・件数を表示しません。</p></fieldset>{error&&<p className="form-error">{error}</p>}<button className="primary-submit" disabled={busy||body.trim().length<2} onClick={submit}>{busy?<LoaderCircle className="spin"/>:existing?"変更を保存":"レビューを残す"}</button></div></Modal>;
}
