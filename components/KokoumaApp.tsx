...
type Review = { id: string; userId: string; placeId: string; rating: number; body: string; imageKey?: string; displayName: string; handle: string; avatarColor: string; isSeed: boolean; isFictionalDemo: boolean; isUnofficial: boolean };
...
function ReviewModal({ place,onClose,onDone }: { place:Place;onClose:()=>void;onDone:()=>void }) {
  const [rating,setRating]=useState(5); const [body,setBody]=useState(""); const [tier,setTier]=useState("S"); const [busy,setBusy]=useState(false); const [error,setError]=useState("");
  const submit=async()=>{setBusy(true);setError("");try{await api("/api/reviews",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({placeId:place.id,rating,body,tier})});onDone();}catch(e){setError((e as Error).message);}finally{setBusy(false)}};
  return <Modal title={`${place.name}の記録`} onClose={onClose}><div className="review-form"><p>あなたの星</p><Stars value={rating} interactive onChange={setRating}/><label>ひとこと<textarea value={body} onChange={(e)=>setBody(e.target.value)} maxLength={600} placeholder="誰と、どんな日に行きたい？ 味だけでなく、あなたの記憶を。"/><small>{body.length}/600</small></label><div><p>あなた的Tier</p><div className="tier-picker">{(["S","A","B","C"] as const).map((t)=><button key={t} className={tier===t?"active":""} style={{"--tier":tierColors[t]} as React.CSSProperties} onClick={()=>setTier(t)}>{t}<small>TIER</small></button>)}</div></div>{error&&<p className="form-error">{error}</p>}<button className="primary-submit" disabled={busy||body.trim().length<2} onClick={submit}>{busy?<LoaderCircle className="spin"/>:"レビューを残す"}</button></div></Modal>;
}
